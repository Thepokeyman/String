...
#if defined(NDEBUG)
# error "String cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6ba598b2ac737f6ebf6c54c062b71c062a49bcb31ecfdd0478ca9b9101717c91");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // String: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "String Signed Message:\n";
...
{
    // String: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 900000); // String: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // String: 3.5 days
static const int64 nTargetSpacing = 10 * 60; // String: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // String: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // String: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x574d6ebfb2c03f96ebfbff7e87be41e8cf418b91bb5a012ea7e4f6a91f01e237");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "North Korean cheerleaders caught off guard by fake Kim Jong Un";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 60 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048e820066945f9225fd54fe8d456ecbd6629023d80980fb495a70e0f3d8f4cb46e7f4e4022d77c5ab58c761ce31ab0302a20f465849848a9e8b54fee3aafc4e1b") << OP_CHECKSIG;
        CBlock block;
        ...
        block.nNonce   = 2084730876;

        if (fTestNet)
        {
            block.nTime    = 1518646627;
            block.nNonce   = 1520758496;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x0441c7242e13eb517db058aa16b8c72a282e6e606fa5a706e26635ed137f7f25"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc4, 0xb2, 0xda }; // String: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("StringMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("StringMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static StringMiner(CWallet *pwallet)
{
    printf("StringMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("string-miner");
    ...
    try { loop {
        ...
        printf("Running StringMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&StringMiner, pwallet));
}
